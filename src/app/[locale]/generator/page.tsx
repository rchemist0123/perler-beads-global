...
export default function Home() {
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (isImageFile || isCsvFile) {
        ...
        alert(t('alerts.unsupportedFileType', { type: file.type || '', name: file.name }));
    ...
    if (fileExtension === 'csv') {
      ...
      importCsvData(file)
        .then(({ mappedPixelData, gridDimensions }) => {
          ...
          alert(t('alerts.csvImportSuccess', { n: gridDimensions.N, m: gridDimensions.M, count: Object.keys(colorCountsMap).length }));
        })
        .catch(error => {
          console.error('CSV导入失败:', error);
          alert(t('alerts.csvImportFailed', { error: error.message }));
        });
    } else {
      // 处理图片文件
  ...
  const pixelateImage = (imageSrc: string, detailLevel: number, threshold: number, currentPalette: PaletteColor[], mode: PixelationMode) => {
    ...
    img.onerror = (error: Event | string) => {
      ...
      alert(t('alerts.cannotLoadImage'));
    ...
    if (originalImageSrc && activeBeadPalette.length > 0) {
       ...
       return () => clearTimeout(timeoutId);
        ...
        if (pixelatedCtx && pixelatedCanvas) {
            ...
            pixelatedCtx.fillText(t('alerts.noAvailableColors'), pixelatedCanvas.width / 2, pixelatedCanvas.height / 2);
        }
        setMappedPixelData(null);
        setGridDimensions(null);
    ...
    const handleToggleExcludeColor = (hexKey: string) => {
        ...
        if (isExcluding) {
            ...
            if (initialGridColorKeys.size === 0) {
                ...
                alert(t('alerts.excludeNotReady'));
                return;
            }
            ...
            if (remapTargetPalette.length === 0) {
                ...
                alert(t('alerts.excludeNoAlternatives', { key: hexKey }));
                console.log("---------");
                return; // 停止排除过程
            }
            ...
            // 检查排除颜色的数据是否存在
             if (!excludedColorData || !mappedPixelData || !gridDimensions) {
                 ...
                 alert(t('alerts.excludeMissingData'));
                console.log("---------");
                 return;
             }
  ...
  const handleAutoRemoveBackground = () => {
    ...
    if (borderCounts.size === 0) {
      alert(t('alerts.noBackgroundEdge'));
      return;
    }
    ...
    if (stack.length === 0) {
      alert(t('alerts.noBackgroundArea'));
      return;
    }
  ...
  const handleExportCustomPalette = () => {
    ...
    if (selectedHexValues.length === 0) {
      alert(t('alerts.noSelectedColors'));
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!Array.isArray(data.selectedHexValues)) {
          throw new Error(t('alerts.invalidFileFormat'));
        }
        ...
        if (invalidHexValues.length > 0) {
          ...
          alert(t('alerts.importInvalidColors', { colors: invalidHexValues.join(', ') }));
        }

        if (validHexValues.length === 0) {
          alert(t('alerts.importNoValidColors'));
          return;
        }
        ...
        alert(t('alerts.importSuccess', { count: validHexValues.length }));

      } catch (error) {
        console.error("导入色板配置失败:", error);
        alert(t('alerts.importFailed', { error: error instanceof Error ? error.message : 'Unknown error' }));
      } finally {
    ...
    <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center bg-gradient-to-b from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
      ...
      <header className="w-full md:max-w-4xl text-center mt-6 mb-8 sm:mt-8 sm:mb-10 relative overflow-hidden">
        ...
        <div className="relative z-10 py-8">
            ...
            <div className="relative flex flex-col items-center space-y-3">
              ...
              <div className="relative">
                <h2 className="relative text-xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-teal-500 via-green-500 to-emerald-400 tracking-widest transform hover:scale-102 transition-all duration-300">
                  {t('header.toolName')}
                  <span className="text-xs font-normal text-gray-400 dark:text-gray-500 tracking-widest ml-1 align-middle">{t('header.subtitle')}</span>
                </h2>
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowDesktopModal(false)}>
              <div className="relative mx-4 w-full max-w-md rounded-2xl border border-blue-200 dark:border-blue-700 bg-white dark:bg-gray-800 p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('header.desktopDetected')}</h3>
                  <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{t('header.desktopRecommendation')}</p>
          ...
          {/* 来源提示 */}
          <p className="mt-2 text-[10px] text-gray-400 dark:text-gray-500">{t('header.sourceAttribution')}</p>
        </div>
      </header>
      ...
      <main ref={mainRef} className="w-full md:max-w-4xl flex flex-col items-center space-y-5 sm:space-y-6 relative overflow-hidden">
        ...
        >
          ...
          {/* Text color */}
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{t('upload.dragDrop')}<span className="font-medium text-blue-600 dark:text-blue-400">{t('upload.clickToSelect')}</span></p>
          {/* Text color */}
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('upload.supportedFormats')}</p>
        </div>
        ...
        {!originalImageSrc && (
          <div className="w-full md:max-w-md bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 p-3 rounded-lg border border-blue-100 dark:border-gray-600 shadow-sm">
            ...
            <p className="text-xs text-indigo-700 dark:text-indigo-300 flex items-start">
              ...
              {/* Text color */}
              <span className="text-indigo-700 dark:text-indigo-300">{t('upload.tip')}</span>
            </p>
          </div>
        )}
            ...
            {!isManualColoringMode && (
              ...
              <div className="w-full md:max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
                ...
                <div className="sm:col-span-2">
                  {/* Label color */}
                  <label htmlFor="pixelationModeSelect" className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 sm:mb-2">{t('controls.processingMode')}</label>
                  <div className="flex items-center gap-2">
                    {/* Select field styles */}
                    <select
                    ...
                    >
                      <option value={PixelationMode.Dominant} className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">{t('controls.cartoonMode')}</option>
                      <option value={PixelationMode.Average} className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">{t('controls.realisticMode')}</option>
                    </select>
                  </div>
                </div>
                ...
                {/* 色号系统选择器 */}
                <div className="sm:col-span-2">
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 sm:mb-2">{t('controls.colorSystem')}</label>
                ...
                <div className="sm:col-span-2 mt-3">
                  ...
                  >
                    ...
                    {t('controls.managePalette')} ({Object.values(customPaletteSelections).filter(Boolean).length} {t('controls.colors')})
                  </button>
                  {isCustomPalette && (
                    <p className="text-xs text-center text-blue-500 dark:text-blue-400 mt-1.5">{t('controls.customPaletteActive')}</p>
                  )}
                </div>
              </div>
            )}
                  ...
                  <div className="flex justify-center">
                    <div className="bg-blue-50 dark:bg-gray-700 border border-blue-100 dark:border-gray-600 rounded-lg p-2 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 text-xs text-gray-600 dark:text-gray-300 w-full sm:w-auto">
                      <div className="flex items-center gap-1 w-full sm:w-auto">
                        ...
                        <span>{t('manualEdit.useMenuTip')}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 w-full sm:w-auto">
                        ...
                        <span>{t('manualEdit.desktopRecommend')}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {gridDimensions && gridDimensions.N > 100 && (
                  <div className="mb-3 p-2 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg text-xs text-blue-700 dark:text-blue-300 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <span>{t('manualEdit.highResGrid', { n: gridDimensions.N, m: gridDimensions.M })} - {t('manualEdit.canvasAutoExpanded')}</span>
                    </div>
                  </div>
                )}
        ...
        {!isManualColoringMode && originalImageSrc && colorCounts && Object.keys(colorCounts).length > 0 && (
          ...
          <div className="w-full md:max-w-2xl mt-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-100 dark:border-gray-700 color-stats-panel">
            ...
            <h3 className="text-lg font-semibold mb-1 text-gray-700 dark:text-gray-200 text-center">
              {t('colorManagement.removeNoise')}
            </h3>
            {/* Subtitle color */}
            <p className="text-xs text-center text-gray-500 dark:text-gray-400 mb-3">{t('colorManagement.clickToExclude')} {t('colorManagement.totalCount', { count: totalBeadCount })}</p>
                ...
                .map((hexKey) => {
                  ...
                  return (
                    <li
                      ...
                      title={isExcluded ? t('colorManagement.clickToRestore', { key: displayColorKey }) : t('colorManagement.clickToExcludeColor', { key: displayColorKey })}
                    >
                      ...
                      <span className={`text-xs ${isExcluded ? 'text-red-600 dark:text-red-400 line-through' : 'text-gray-600 dark:text-gray-300'}`}>{count} {t('common.beads')}</span>
                    </li>
                  );
                })}
            </ul>
            {excludedColorKeys.size > 0 && (
                <div className="mt-3">
                  ...
                  >
                    <span>{t('colorManagement.excludedColors', { count: excludedColorKeys.size })}</span>
                    <svg 
        ...
        {/* Message if palette becomes empty (Also hide in manual mode) */}
         {!isManualColoringMode && originalImageSrc && activeBeadPalette.length === 0 && excludedColorKeys.size > 0 && (
             ...
             <div className="w-full md:max-w-2xl mt-6 bg-yellow-100 dark:bg-yellow-900/50 p-4 rounded-lg shadow border border-yellow-200 dark:border-yellow-800/60 text-center text-sm text-yellow-800 dark:text-yellow-300">
                 {t('colorManagement.tooFewColors')}
                 {excludedColorKeys.size > 0 && (
                      ...
                      >
                          {t('colorManagement.viewExcluded', { count: excludedColorKeys.size })}
                      </button>
                  )}
             </div>
         )}
